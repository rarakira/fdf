...
t_point	**create_map(void)
{
	...
	*(ptr + 10) = NULL;
	...
	{
		j = 0;
		*(ptr + i) = malloc(sizeof(t_point *) * 5);
		if (!*(ptr + i))
			exit(1);
		while (j < 5)
		{
			//printf("i = %d, j = %d, *(ptr + i) = %p, %zu\n", i, j, *(ptr + i), sizeof(*(ptr + i)));
			//printf("(ptr + i)[j] = %p, %zu\n", (ptr + i)[i], sizeof((ptr + i)[i]));
			(ptr + i)[j] = malloc(sizeof(t_point));
			(ptr + i)[j]->x = i;
			(ptr + i)[j]->y = j;
			(ptr + i)[j]->z = 0;
			printf("[%p] %d & %d\n", (ptr + i)[j], i, j);
			printf("(ptr + %d)[%d]->x = %d, (ptr + i)[j]->x = %d, (ptr + i)[j]->x = %d\n", i, j, (ptr + i)[j]->x, (ptr + i)[j]->y, (ptr + i)[j]->z);
			j++;
		}
		i++;
	}
...
int main(void)
{
	...
	for (int k = 0; k < 11; k++)
	{
		for (int m = 0; m < 5; m++)
		{
			...
			if (m < 4) // draw horizontal
			{
				...
				x_start = map_point_x((ptr + k)[m]);
				y_start = map_point_y((ptr + k)[m]);
				x_fin = map_point_x((ptr + k)[m + 1]);
				y_fin = map_point_y((ptr + k)[m + 1]);
			...
			if (k < 10) // draw vertical
			{
				printf("point1 %p, point2 %p\n", (ptr + k)[m], (ptr + k + 1)[m]);
				x_start = map_point_x((ptr + k)[m]);
				y_start = map_point_y((ptr + k)[m]);
				x_fin = map_point_x((ptr + k + 1)[m]);
				y_fin = map_point_y((ptr + k + 1)[m]);
	...
	for (int k = 0; k < 11; k++)
	{
		for (int m = 0; m < 5; m++)
		{
			printf("point %p: x = %d, y = %d, z = %d\n", (ptr + k)[m], (ptr + k)[m]->x, (ptr + k)[m]->y, (ptr + k)[m]->z);
		}
	}
